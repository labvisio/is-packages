...
class PrometheusCpp(ConanFile):
    ...
    def export_sources(self):
        export_conandata_patches(conanfile=self)

    def requirements(self):
        self.requires("zlib/1.2.13")

    def source(self):
        git = Git(conanfile=self)
        git.clone(url="https://github.com/jupp0r/prometheus-cpp", target=".")
        git.run("checkout v0.4.1")
        git.run("submodule update --init")
        apply_conandata_patches(conanfile=self)

    def layout(self):
        cmake_layout(self, src_folder="src")

    def generate(self):
        cmake = CMakeToolchain(conanfile=self)
    ...
    def package_info(self):
        ...
        self.cpp_info.components["prometheus-cpp"].set_property("cmake_target_name",
